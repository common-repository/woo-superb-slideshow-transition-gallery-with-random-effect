...
<div class="wrap">
  <div class="form-wrap">
	...
	if (isset($_POST['woo_form_submit']) && $_POST['woo_form_submit'] == 'yes')
	{
		...
		$woo_title = stripslashes(sanitize_text_field($_POST['woo_title']));
		$woo_pause = intval($_POST['woo_pause']);
		$woo_transduration = intval($_POST['woo_transduration']);
		$woo_random = stripslashes(sanitize_text_field($_POST['woo_random']));
		$woo_type = stripslashes(sanitize_text_field($_POST['woo_type']));
		
		if(!is_numeric($woo_pause) || $woo_pause == 0) { $woo_pause = 2000; }
		if(!is_numeric($woo_transduration) || $woo_transduration == 0) { $woo_transduration = 1000; }

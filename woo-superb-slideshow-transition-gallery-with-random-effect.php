...
if ( ! defined( 'WP_woo_PLUGIN_URL' ) )
	define( 'WP_woo_PLUGIN_URL', plugins_url() . '/' . WP_woo_PLUGIN_NAME );
	
if ( ! defined( 'WP_woo_ADMIN_URL' ) )
	define( 'WP_woo_ADMIN_URL', admin_url() . 'options-general.php?page=woo-superb-slideshow' );
...
function woo_shortcode( $atts ) 
{
	...
	if ( ! empty($data) ) 
	{
		...
		if (!isset($Woo_ScriptInserted) || $Woo_ScriptInserted !== true)
		{
			...
			$woo_xml = $woo_xml .'<link rel="stylesheet" href="'.WP_woo_PLUGIN_URL.'style.css" type="text/css" />';
		}
...
function woo_adminscripts() 
{
	if( !empty( $_GET['page'] ) ) 
	{
		switch ( $_GET['page'] ) 
		{
			case 'woo-superb-slideshow':
				wp_register_script( 'woo-adminscripts', plugins_url( 'pages/setting.js', __FILE__ ), '', '', true );
				wp_enqueue_script( 'woo-adminscripts' );
				$woo_select_params = array(
					'woo_path'   	=> __( 'Please enter the image path.', 'woo-select', 'woo-transition' ),
					'woo_link'   	=> __( 'Please enter the target link.', 'woo-select', 'woo-transition' ),
					'woo_target' 	=> __( 'Please select the target status.', 'woo-select', 'woo-transition' ),
					'woo_title'  	=> __( 'Please enter the image title.', 'woo-select', 'woo-transition' ),
					'woo_type'  	=> __( 'Please select the gallery type.', 'woo-select', 'woo-transition' ),
					'woo_status' 	=> __( 'Please select the display status.', 'woo-select', 'woo-transition' ),
					'woo_order'  	=> __( 'Please enter the display order, only number.', 'woo-select', 'woo-transition' ),
					'woo_delete'	=> __( 'Do you want to delete this record?', 'woo-select', 'woo-transition' ),
				);
				wp_localize_script( 'woo-adminscripts', 'woo_adminscripts', $woo_select_params );
				break;
		}
	}
}
...
add_action( 'admin_enqueue_scripts', 'woo_adminscripts' );
?>